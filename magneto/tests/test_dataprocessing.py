...
import magneto
from magneto.data_processing import _check_good_tel


def test_data_select_wl(example_oifits_rgrav):
    d = magneto.load(example_oifits_rgrav)
    ...
    d_sel = magneto.select_data(d, **param_sel)
    npts_sel = magneto.get_stat_data([d_sel])
...
def test_check_bl(example_merge_ref):
    d = magneto.load(example_merge_ref)
...
def test_data_temp_bin(example_t1, example_t2, example_t3, example_merge_ref):
    ...
    list_d = [magneto.load(x) for x in list_file]
    d_ref = magneto.load(example_merge_ref)
    d_bin = magneto.temporal_bin_data(list_d)
    bl1_ref = d_ref.bl[0]
    bl2_ref = d_ref.bl[2]
...
def test_data_spec_bin(example_oifits_rgrav):
    ...
    d_bin = magneto.spectral_bin_data(d, nbox=nbox)

    # Flag
    wl_min = 2.0
    wl_max = 2.2
    d_bin_flag = magneto.spectral_bin_data(d, nbox=nbox, wave_lim=[wl_min, wl_max])
